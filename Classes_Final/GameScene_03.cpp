...
void GameScene_03::createMenu()
{
	auto btn_Pause = MenuItemImage::create("Game/btn_pause.png","Game/btn_pause_std.png",
		CC_CALLBACK_1(GameScene_03::menuPause, this));
	auto menu = Menu::createWithItem(btn_Pause);
	menu->setPosition(50,visibleSize.height - 30);
	this->addChild(menu);
}

void GameScene_03::menuPause(Ref* pSender)
{
	Director::getInstance()->pushScene(Pause::createScene(3));
}

Scene* GameScene_03::createScene()
{
	// 'scene' is an autorelease object
	auto scene = Scene::create();
	// 'layer' is an autorelease object
	auto layer = GameScene_03::create();
...
void GameScene_03::createSlider()
{
...
void GameScene_03::update(float delta)
{
	int num = 0;
	int addBalls = 0;
	...
	if (bricks->empty())
	{
		Director::getInstance()->replaceScene(Win::createScene(3));
		unscheduleUpdate();
	}
	else if (balls->empty())
	{
		Director::getInstance()->replaceScene(Fail::createScene(3));
		unscheduleUpdate();
	}
	//±éÀúÃ¿Ò»¸öÇò
	for( auto iter1 = balls->begin(); iter1 != balls->end(); iter1++)
	{
		(*iter1)->setballvisible(slider);
...
void GameScene_03::menuCloseCallback(Ref* pSender)
{
