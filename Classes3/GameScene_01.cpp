...
void GameScene_01::createMenu()
{
	...
	menu->setPosition(50,visibleSize.height - 50);
	this->addChild(menu);
}
...
void GameScene_01::creatBricks()
{
	...
	int num_row = visibleSize.height / b_h / 2 - 1;
	int brickBf = random<int>(1, 6);
	for ( int i=0; i<num_line; i++)
	{
		for ( int j=0; j<num_row; j++)
		{
			brick->setPosition(offset + i*b_w + b_w/2, visibleSize.height - j*b_h - b_h/2 );
			int brickBf = random<int>(1, 6);
			switch (brickBf)
			{
			case 1:
		...
		for (int i=0; i<(*iter1)->get_speed(); i++)
		{
			...
			for (auto iter = bricks->begin(); iter != bricks->end(); iter++)
			{
				if((*iter1)->isCollideWithBrick(*iter))
				{
					...
					//log("remove this brick:position:x:%f,y:%f,width:%f",(*iter)->getPositionX(),(*iter)->getPositionX(),(*iter)->getContentSize().width);
					...
					if ( ADD_LENGTH == (*iter)->getBf())
					{
						...
						//texture = TextureCache::sharedTextureCache()->addImage("Game/level_1/paddle_long.png");
						//slider->setTexture(Director::getInstance()->getTextureCache()->addImage("Game/level_1/paddle_long.png"));
						slider->setTexture("Game/level_1/paddle_long.png");
					}
					if ( SUB_LENGTH == (*iter)->getBf())
					{
						//texture = TextureCache::sharedTextureCache()->addImage("Game/level_1/paddle_short.png");
						//slider->setTexture(Director::getInstance()->getTextureCache()->addImage("Game/level_1/paddle_short.png"));
